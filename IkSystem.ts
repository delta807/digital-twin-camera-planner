...
export class IkSystem {
    ...
    constructor(mujoco: MujocoModule, camera: THREE.Camera, domElement: HTMLElement, orbitControls: OrbitControls) {
        ...
        // three 0.181: TransformControls extends Controls (not Object3D) and is no
        // longer renderable itself. getHelper() returns the Object3D holding the gizmo
        // meshes, which is what gets added to the scene. (Mirrors WorkspaceCameraRig.)
    ...
    setGizmoVisible(visible: boolean) {
        // The helper carries the renderable gizmo meshes; toggle its visibility and
        // disable the control so it stops responding to pointer events when hidden.
        this.helper.visible = this.control.enabled = visible;
    }
