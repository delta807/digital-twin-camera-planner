...
export function App() {
  ...
  // edit that triggered it already re-renders App). Avoids the per-edit "double render" amplification
  // (history-recording effect that setStates on every change) that made editing feel laggy.
  const availRef = useRef({ u: false, r: false });
  const refreshHistButtons = () => {
  ...
  const handleStationChange = (id: string, patch: Partial<{ x: number; y: number; yaw: number; shapeSides: number; length: number; width: number; postX: number; postY: number; postHeight: number; sideExtents: [number, number, number, number]; cornerRadii: number[]; railLengths: number[]; railLinks: number[] }>) => {
    ...
    if (patch.length !== undefined || patch.width !== undefined || patch.shapeSides !== undefined) {
      patch = { sideExtents: undefined, cornerRadii: undefined, railLengths: undefined, railLinks: undefined, ...patch } as typeof patch;
    }
    ...
    if (id === 'primary') {
      ...
      handleWorkcellChange({
        ...
        ...('railLinks' in patch ? { railLinks: patch.railLinks } : {}),
      });
          ...
          {(() => {
            const inspectorEl = (inline: boolean) => (
              <SelectionInspector
                ...
                station={(() => {
                  if (selection?.stationId === 'primary') { const w = workcellConfig; return { x: w.originX ?? 0, y: w.originY ?? 0, yaw: w.yaw ?? 0, shapeSides: w.shapeSides, length: w.length, width: w.width, sideExtents: w.sideExtents, cornerRadii: w.cornerRadii, railLengths: w.railLengths, railLinks: w.railLinks }; }
