...
export type FigureKey = 'reach' | 'coverage' | 'depth' | 'conflict' | 'layout' | 'manip' | 'effort' | 'gsd' | 'handoff' | 'cycle' | 'throughput';
...
export const ANALYSES: Analysis[] = [
  ...
  { n: 4, title: 'Cycle time / throughput', purpose: 'Layout speed — pick → place → retreat time per cycle.', metric: 'trapezoidal joint time, slowest-joint synced', status: 'live', icon: Timer, figure: 'cycle' },
  { n: 5, title: 'Resolution (GSD) map', purpose: 'How fine (mm/px) the camera resolves across the table.', metric: '2·z·tan(HFOV/2) / h_px', status: 'live', icon: Scan, figure: 'gsd' },
  ...
  { n: 9, title: 'Handoff feasibility', purpose: 'Where the two arms can actually exchange an object.', metric: 'bimanual reachable ∩ collision-free', status: 'live', icon: Hand, figure: 'handoff' },
  { n: 10, title: '1-vs-2 arm throughput', purpose: 'Whether a second arm is worth the added cost.', metric: 'throughput gain vs collision cost', status: 'live', icon: TrendingUp, figure: 'throughput' },
...
export function AnalysisCatalog({ isDarkMode, onSelect, scopeLabel, armsInScope }: { isDarkMode: boolean; onSelect: (figure: FigureKey) => void; scopeLabel: string; armsInScope: number }) {
  ...
  const needsTwo = (n: number) => (n === 7 || n === 8 || n === 9 || n === 10) && armsInScope < 2;
