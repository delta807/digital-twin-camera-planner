...
export interface DockArmsProps {
  ...
  onAnalysis: () => void; // open the analysis panel (reachability / depth / coverage figures)
}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-black/5">
        ...
        <Section title="Scene" icon={<Grid3x3 className="w-3.5 h-3.5 text-indigo-500" />} isDarkMode={isDarkMode}>
          ...
          <div className={`pt-1.5 mt-1 border-t ${isDarkMode ? 'border-white/10' : 'border-black/5'} space-y-1.5`}>
            ...
            <button onClick={arms.onAnalysis} className={`w-full py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wide ${isDarkMode ? 'bg-white/5 text-slate-200 hover:bg-white/10' : 'bg-black/5 text-slate-700 hover:bg-black/10'}`} title="Reachability / depth / camera-coverage figures for the live layout (with PNG export)">Analysis & export ▸</button>
          </div>
        </Section>
      </div>
    </div>
