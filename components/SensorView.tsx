 ...
 */
export function SensorView({ canvasHostRef, isDarkMode, sidebarOpen, aspect, onClose }: SensorViewProps) {
  ...
  return (
    <div className={`absolute top-6 right-6 ${rightClass} z-30 w-72 max-[660px]:w-56 rounded-2xl glass-panel shadow-xl overflow-hidden ${panelStyle}`}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-black/5">
        <div className="flex items-center gap-2">
          <Video className="w-3.5 h-3.5 text-indigo-500" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Sensor View · D435i</span>
        </div>
