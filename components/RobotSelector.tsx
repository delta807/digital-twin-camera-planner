 ...
 */
export function RobotSelector({ gizmoStats, isDarkMode, robotName = 'Franka Panda' }: RobotSelectorProps) {
  ...
  return (
    // Top-center so it clears the side panels (Camera Planner left, analysis right).
    <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-4">
      <div className={`glass-panel px-8 py-4 rounded-[2rem] min-w-[240px] shadow-2xl ${panelStyle}`}>
        <h1 className="text-xl font-bold tracking-tight leading-none text-center">{robotName}</h1>
      </div>
