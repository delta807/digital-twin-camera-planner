/**
 * Vendored from urdf-loaders (gkjohnson) — javascript/src/URDFDragControls.js
 * https://github.com/gkjohnson/urdf-loaders  · License: Apache-2.0
 *
 * This is the URDF-AGNOSTIC interaction engine leLab uses (via <urdf-viewer>): raycast a mesh,
 * walk parents to the nearest object with `isURDFJoint && jointType !== 'fixed'`, and drag to
 * rotate it about its axis (plane-projection angle math). It touches NO URDF/loader/meshes — it
 * only reads `joint.axis`, `joint.matrixWorld`, `joint.angle` and calls `updateJoint()`. We adapt
 * it to MuJoCo by duck-typing our body groups as joints (see MujocoJointDrag.ts).
